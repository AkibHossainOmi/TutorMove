...
const ProfileHeader = ({ profile }) => {
  const { userData, userType, isEditing, toggleEdit, handleProfileFileChange } = profile;
        ...
        <div className="relative">
          ...
          {isEditing && (
            <label className="absolute bottom-0 right-0 bg-indigo-600 text-white p-1 rounded-full cursor-pointer hover:bg-indigo-700">
              <FaCamera />
              <input type="file" className="hidden" onChange={e => handleProfileFileChange(e.target.files[0])} />
            </label>
          )}
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
        <button
          ...
          className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg font-medium transition w-full sm:w-auto ${
            isEditing ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-white text-indigo-700 border border-indigo-700 hover:bg-indigo-50'
          }`}
        >
          {isEditing ? (
            <>
