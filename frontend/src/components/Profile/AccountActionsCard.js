...
const AccountActionsCard = ({ profile }) => {
  ...
  const handleVerifyPassword = async () => {
    ...
    try {
      // Use the same API method as password change to verify the current password
      // We'll send a dummy password change request just to verify the current password
      // The backend will validate the current password before attempting to change it
      await userApi.updatePassword({
        currentPassword: deletePassword,
        newPassword: deletePassword, // Same as current, just for verification
      });
      setPasswordVerified(true);
      setDeleteError('');
    } catch (err) {
      // If the error is about current password being incorrect
      if (err.message && (err.message.includes('current password') || err.message.includes('incorrect') || err.message.includes('wrong'))) {
        setDeleteError('Incorrect password. Please try again.');
      } else {
        // If it's another error (like "new password must be different"), we know the current password was correct
        setPasswordVerified(true);
        setDeleteError('');
      }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      // Pass the verified password to the delete account API
